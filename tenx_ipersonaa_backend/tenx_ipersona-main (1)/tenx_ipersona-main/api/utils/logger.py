...
#optimal_log_level = logging.INFO
api_optimal_log_level = logging.INFO
leap_optimal_log_level = logging.CRITICAL
...
class LLPackerLogger(object, metaclass=SingletonType):
    ...
    def __init__(self, log_file_name, 
        ...
        self.good_log_level = 40
        self.fail_log_level = 50
        
        self._logger = logging.getLogger(log_file_name)  
        filename = os.path.basename(log_file_name)
        
        # Set log level      
        if filename.startswith('leap_'):
            self._logger.setLevel(leap_optimal_log_level)         
        elif 'pages/' in log_file_name:
            self._logger.setLevel(api_optimal_log_level)
        else:             
        ...
        formatter = logging.Formatter(f'{filename} %(asctime)s \t %(message)s')
    ...
    def get_log_level_value(self, log_level):
        if isinstance(log_level, str):
            if hasattr(logging, log_level.upper()):
                log_level = getattr(logging, log_level.upper())
    ...
    def compose_log_message(self, *args, **kwargs):  
        ...
        for k in ['arrow','smallarrow','bigarrow', 'achar']:
            _ = kwargs.pop(k, None)
    ...
    def divider(self, *message, **kwargs):
        if kwargs.get('level',self.good_log_level) >= self.log_level:
    ...
    def fail(self, *message, **kwargs):
        if kwargs.get('level',self.fail_log_level) >= self.log_level:
...
if __name__ == "__main__":
    logger = LLPackerLogger("test.log", bucket_name="all-tenx-system-logs", s3_prefix='tenx-logs')
