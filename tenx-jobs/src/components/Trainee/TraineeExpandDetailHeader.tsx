...
export default function TraineeExpandDetailHeader() {
    ...
    const handleNavigateBackward = () => {
      if (currentIndex > 0) {
        ...
        navigate(`/trainee/trainee_engagements/${all_user_id}/${user_profile_id}/${prevId}`);
      }
    };
    ...
    const handleNavigateForward = () => {
      if (currentIndex < reactionIds.length - 1) {
        ...
        navigate(`/trainee/trainee_engagements/${all_user_id}/${user_profile_id}/${nextId}`);    }
    };
  
  return (
    <>
        <Col xs={24} lg={20} xxl={16} className="expand__header-column">
                <Flex gap={"0.5rem"} style={{padding:"0.75rem 1rem", background: "#FFF",border: "2px solid D9D9D9", borderRadius: "0.5rem", marginBottom: "1rem"}}>
                    <ArrowLeftOutlined className="engagement-detail-icon" onClick={() => navigate(`/trainee/my-jobs`)}/>
            ...
            <Card className="trainee-container-column" style={{borderRadius: "8px 8px 0 0"}}>
                <Flex justify={'space-between'} gap={"1.5rem"}>
                    <p className="engagement-detail-navigation" style={{ width: "100%" }}>
                        {user_reaction_id ? userReactionIds[user_reaction_id] : "No reaction selected"}
                    </p>
