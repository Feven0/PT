...
Table
  ...
  export default function TableExtension<T extends object>(props: AntdTableProps<T>) {
    ...
    const { dataSource, columns, expandable, allowEditColumn, onChange,
            ...
            buttons, dropDown, switchComp, email, scroll, rowClassName, setSelectedParentRows,
        } = props;
    const { downloadPermission, fileName, columnPicker } = props.download ?? {};
    const { showSizeChanger, paginationSize, pagination, setPaginationSize, total, current } = props.pagination;
  ...
  const handleColumnsShowChange = (showColumns: string[]): void => {
    ...
    const tableProps = {
      ...
      columns: [
        {
          ...
          render: (_text: string, _record: any, index: number) => {
            ...
            return (currentPage - 1) * (pagination ?? 1) + index + 1;
        },
      },
      ...filteredColumns
      ...
      scroll: scroll, 
      pagination: {
        defaultPageSize: paginationSize || 25,
        ...
        <Paragraph style={{ marginRight: '10px', marginTop: "0.1rem"}}>
          {
            (selectedRowKeys.length > 0
              ...
              : getSingularPluralString(dataSource?.length, counterName))
            }
        </Paragraph>
