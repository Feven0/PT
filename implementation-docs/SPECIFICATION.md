...
- **Speech-to-Text** (Priority Order): 
  - **Google Cloud Speech-to-Text** (PRIMARY - real-time interviews)
  - AssemblyAI (batch processing)
...
Socket.IO: "audio transcribe google"
```
- **Primary transcription service for real-time audio interviews**
...
**4. Google Gemini Speech-to-Text**
```python
POST /api/ipersona/stt/gemini-upload
```
- Uses Google Gemini API for transcription
- Returns transcript with timestamps
- Alternative Google service

**5. AssemblyAI**
```python
Socket.IO: "audio transcribe"
Event: "assemblyai_status"
```
- Used for batch audio processing
- Returns transcript with speaker diarization
- Good for uploaded audio files
- Provides detailed analysis

**STT Selection Logic:**

**For Real-Time Interviews (Primary Use Case):**
1. **Google Cloud STT** (PRIMARY) - Used in `audio chat sentence` Socket.IO event
2. Fallback to Faster Whisper if Google Cloud unavailable
3. Fallback to OpenAI Whisper API

**For Uploaded Audio Files:**
1. AssemblyAI for detailed analysis
2. Google Cloud STT for quick transcription
3. Faster Whisper for local processing

**For Testing/Development:**
- Faster Whisper (no API costs)
- Google Cloud STT (for production-like testing)
...
- `audio transcribe google` - **PRIMARY: Real-time transcription using Google Cloud STT**
- `audio chat sentence` - **Real-time interview with AI evaluation (uses Google Cloud STT)**
- `interview chat` - Text-based interview chat
- `audio transcribe whisper` - Transcribe audio using Faster Whisper (alternative)
- `audio transcribe` - AssemblyAI transcription event (for uploaded files)
...
- `POST /api/ipersona/stt/google-upload` - **Google Cloud STT (PRIMARY)**
...
- `POST /api/ipersona/stt/gemini-upload` - Google Gemini STT (alternative)
- `POST /api/ipersona/audio_upload` - Main audio transcription endpoint

**Socket.IO Events for STT:**
- `audio transcribe google` - **Primary real-time transcription** (Google Cloud STT)
- `audio transcribe whisper` - Faster Whisper streaming
- `audio transcribe` - AssemblyAI batch processing
...
1. **Add new feature**: Update spec in `SPECIFICATION.md`
...
```
Read and explain [the specification](./SPECIFICATION.md)
```

**To make modifications:**
```
Update [feature X] according to the specification in ./SPECIFICATION.md
...
**For New Features:**
1. Update `SPECIFICATION.md` with new feature specification
2. Use AI agent with `COMPILE_GUIDE.md` to implement
3. Test thoroughly
...
**For Onboarding:**
- Read `SPECIFICATION.md` to understand entire system architecture
